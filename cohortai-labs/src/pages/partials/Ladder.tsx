      ...
      {site.ladder.map((s, idx) => (
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-xs tracking-[0.22em] uppercase text-cyan-300/80">{s.duration}</div>
            ...
            <div className="shrink-0 rounded-2xl px-4 py-3 bg-gradient-to-r from-cyan-300/15 via-violet-300/10 to-emerald-300/15 border border-slate-700/60">
              ...
              <div className="text-xs text-slate-500 mt-1">Starting</div>
            </div>
          </div>

          <div className="mt-5 grid gap-2 text-sm text-slate-200">
            {s.includes.map((i) => (
              <div key={i} className="rounded-2xl bg-white/5 px-4 py-3 border border-slate-700/60">• {i}</div>
            ))}
          </div>
        </motion.div>
      ))}
